...
/**
 * Control 2 at runtime — the provision whitelist, applied to the model's prose.
 *
 * Quote verification (below) governs what the model may claim about the
 * POLICY. It says nothing about what the model may claim about the LAW, and
 * that is the more dangerous direction: free prose can attach a real provision
 * number to the wrong duty — "Art. 30(5) requires a DPO above 250 employees" —
 * which reads as authoritative precisely because the article and the number are
 * both genuine.
 *
 * So any provision the model names in prose is checked against the ids actually
 * ingested for that law. Anything unrecognised is flagged, and the sentence
 * carrying it is withheld rather than shown. This is an id lookup, not a
 * judgement about whether the reasoning is sound.
 *
 * `known` is the array of provision ids for the law, from
 * lib/data/provision-index.json.
 */
export function screenCitations(prose, lawId, known, extractCitations) {
  if (!prose || !known?.length) return { text: prose, stripped: [] };
  const allow = new Set(known);
  const stripped = [];

  // Sentence granularity: dropping the whole answer over one bad reference
  // loses good reasoning, and dropping only the number leaves a claim with no
  // support at all.
  const sentences = String(prose).split(/(?<=[.!?])\s+/);
  const kept = sentences.filter((s) => {
    const cited = extractCitations(s, lawId);
    if (!cited.length) return true;
    const bad = cited.filter((k) => !resolveAgainst(k, allow));
    if (bad.length) {
      stripped.push({ sentence: s.trim(), provisions: bad });
      return false;
    }
    return true;
  });
...
/** Ancestor-aware membership: a sub-clause is covered by its parent's chunk. */
function resolveAgainst(key, allow) {
  const m = String(key).match(/^([a-z0-9]+-)(.+)$/);
  if (!m) return allow.has(key);
  let [, prefix, num] = m;
  for (;;) {
    if (allow.has(prefix + num)) return true;
 ...
 */
export function verifyAgainstPolicy(results, policyText, opts = {}) {
  const haystack = normalize(policyText);
  // Control 2 inputs, when the caller supplies them. Optional so the pure
  // quote check still works standalone.
  const { lawId, knownProvisions, extractCitations } = opts;
  const screen = (prose) =>
    lawId && knownProvisions && extractCitations
      ? screenCitations(prose, lawId, knownProvisions, extractCitations)
      : { text: prose, stripped: [] };
  ...
  return results.map((r) => {
    ...
    const reason = screen(typeof r.reason === "string" ? r.reason : "");
    const missing = screen(
      typeof r.missing === "string" && r.missing.trim() ? r.missing : null,
    );
    ...
    return {
      ...
      reason: reason.text,
      missing: missing.text || null,
      // Provisions the model named that this corpus does not contain. Surfaced,
      // not swallowed: a reader should see that something was withheld.
      strippedCitations: [...reason.stripped, ...missing.stripped],
    };
  });
}
