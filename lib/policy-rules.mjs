...
function parseHeading(line) {
  ...
  // Markdown: ## Access and Choice
  const md = l.match(/^#{1,6}\s+(.+?)\s*#*$/);
  if (md) return { number: null, title: md[1].replace(/\*\*/g, "").trim() };

  // Numbered: "4.1 Your rights" / "4.1. Your rights" / "Section 4.1 — Your rights"
  const num = l.match(/^(?:section\s+)?(\d+(?:\.\d+)*)[.):]?\s*[—–-]?\s*(.{2,80})$/i);
  if (num && !/[.!?]$/.test(num[2])) return { number: num[1], title: num[2].replace(/\*\*/g, "").trim() };
...
export function gapStatement(f, shortName) {
  ...
  if (!f.evidence.length) {
    const base =
      `No clause matching this obligation was located, so there is no existing wording to amend — ` +
      `${shortName} addresses it and the policy appears silent.`;
