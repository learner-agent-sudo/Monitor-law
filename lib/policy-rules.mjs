...
export const LANES = {
  ACT: {
    id: "ACT",
    title: "Missing from your policy — add wording",
    tip: "The law requires this and nothing in your policy addresses it.",
    blurb:
      "Confirm the organisation actually does this first. Then publish wording that describes the real practice — never the other way round.",
  },
  REVIEW: {
    id: "REVIEW",
    title: "Already in your policy — check it goes far enough",
    tip: "Wording addressing this was found. Located, not judged.",
    blurb:
      "Read the quoted clause against the statutory text beside it. Ask whether it carries the specifics the law demands — timeframes, categories, named mechanisms. Vague wording can be present and still fall short.",
  },
  CONSIDER: {
    id: "CONSIDER",
    title: "Optional under this law — your judgement",
    tip: "Not required here, or only through regulator guidance. Declining is legitimate.",
    blurb:
      "This law addresses the topic weakly, or imposes something narrower than the requirement as this taxonomy defines it. Not a deficiency — but often mandatory under a different law in the catalog, so worth doing anyway. Check the basis tag on each step: a narrower sub-duty may still bind you.",
  },
  ELSEWHERE: {
    id: "ELSEWHERE",
    title: "Not a policy question — check your paperwork",
    tip: "Lives in contracts, registers and procedures, not in a published policy.",
    blurb:
      "A real obligation, discharged through documents the public never sees. Adding policy wording proves nothing here — go and check the processor contract, the processing register, the impact assessment, the breach procedure.",
  },
  NONE: {
    id: "NONE",
    title: "This law does not require it",
    tip: "Nothing to do.",
    blurb:
      "Listed only so the same set of requirements stays comparable across laws — another law in the catalog may well require it.",
  },
};

/**
 * How a finding was actually decided.
 *
 * This matters more than it first appears. Policies almost never reuse the
 * statute's vocabulary — they implement a duty in their own words — so keyword
 * matching is weak evidence in BOTH directions: a miss usually means different
 * wording rather than a real gap, and a hit can be an unrelated sentence that
 * happened to contain the word. Saying which pass decided a finding is the
 * difference between a result a reader can weigh and one they cannot.
 */
export const CONFIDENCE = {
  read: {
    ...
    label: "Read by AI",
    blurb:
      "A model read your policy for this obligation, and every quote it gave was verified word-for-word against your text before being shown.",
  },
  pattern: {
    id: "pattern",
    label: "Keyword match only",
    blurb:
      "Decided by keyword patterns alone — no model has read this one. Policies usually implement a law in their own words, so treat this as a prompt to look rather than an answer.",
  },
};
...
/** Loose comparison for locating a model's quote back in the document. */
function normText(s) {
  return String(s)
    .replace(/[‘’ʼ]/g, "'")
    .replace(/[“”]/g, '"')
...
/**
 * Fold the model's answer INTO the finding, rather than bolting it on beside.
 *
 * The keyword pass and the reading pass were previously shown as two separate
 * opinions, leaving the reader to reconcile them. That gets the weighting
 * backwards. A policy almost never borrows the statute's vocabulary — it
 * implements the duty in its own words — so when a model has actually read the
 * text, its answer IS the answer, and the keyword result was only ever a cheap
 * way of deciding what to send it.
 *
 * So a re-opened gap does not merely gain a note: the verdict changes, and the
 * finding MOVES to the lane that now describes it. Something the model found
 * belongs under "already in your policy", not under "missing" with a footnote.
 *
 * The safety rules from the interpretation layer survive unchanged:
 *   - only a claim carrying quotes verified against the document can upgrade
 *     a finding; `unverified` leaves the original gap standing
 *   - `absent` never downgrades anything — it confirms the gap, which is a
 *     useful answer in its own right
 *   - policyScope still caps the result, so an obligation a policy can only
 *     partly evidence stays PARTIAL even when a model reads it as addressed
 */
export function consolidate(findings, interpretations = {}, policyText = "") {
  const sections = policyText ? toSections(policyText) : [];
  const locate = (quote) => {
    const n = normText(quote);
    return sections.find((s) => normText(s.text).includes(n))?.label ?? null;
  };

  const merged = findings.map((f) => {
    const ip = interpretations[f.id];
    if (!ip) return { ...f, confidence: "pattern" };

    const base = {
      ...f,
      confidence: "read",
      modelReason: ip.reason || null,
      modelMissing: ip.missing || null,
      rejectedQuotes: ip.rejectedQuotes ?? [],
    };

    // The model found nothing either. Two independent passes agreeing does not
    // prove a gap, but it is materially better evidence than one.
    if (ip.verdict === "absent") return { ...base, modelAgreedAbsent: true };

    // A claim whose quotes did not survive verification changes nothing.
    if (ip.verdict === "unverified") return { ...base, modelRejected: true };

    const scope = policyScope[f.id]?.scope ?? "assessable";
    const verdict = ip.verdict === "partially" || scope === "partial" ? "PARTIAL" : "EVIDENCED";

    const modelEvidence = ip.quotes.map((q) => ({
      text: q,
      section: locate(q),
      source: "model",
    }));
    const evidence = [...modelEvidence, ...f.evidence.map((e) => ({ ...e, source: "pattern" }))];
    ...
    return {
      ...
      evidence,
      editTarget: evidence[0]?.section ?? null,
      lane: laneFor(verdict, f.strictness),
      priority: priority(verdict, f.strictness),
    };
  });
...
export function gapStatement(f, shortName) {
  ...
  if (!f.evidence.length) {
    // A model has read it and agrees nothing is there. That is a stronger
    // statement than a keyword miss and should not be phrased like one.
    if (f.modelAgreedAbsent) {
      const why = f.modelReason ? ` ${f.modelReason}` : "";
      return (
        `Keyword search found nothing, and a model that read the whole policy also found nothing ` +
        `addressing this.${why} Two independent passes agreeing is the strongest signal this tool ` +
        `produces — though still not proof, since the practice may exist and simply go undescribed.`
      );
    }
    if (f.modelRejected) {
      return (
        `A model claimed this was addressed, but the wording it quoted does not appear in your ` +
        `document, so the claim was discarded and the gap stands. Nothing here has been shown to ` +
        `address ${shortName} on this point.`
      );
    }
    const base =
      ...
      (f.confidence === "pattern"
        ? ` This was decided by keyword matching alone; policies usually use their own words, so a model reading it may well find something.`
        : "");
  ...
  // Where a model located the clause, lead with what it said — it read the
  // document, and the element checklist is still only keyword matching.
  const lead = f.modelReason ? `${f.modelReason.replace(/\s*$/, "")} ` : "";
  const stillMissing = f.modelMissing
    ? ` The model notes what is still not covered: ${f.modelMissing.replace(/\.\s*$/, "")}.`
    : "";

  if (!f.elements.length) {
    return `${lead}Read the clause above against the statutory text to judge whether it goes far enough.${stillMissing}`;
  }
  if (!f.missingElements.length) {
    return `${lead}Every element this tool checks for is present somewhere in the policy. That is not a finding of adequacy — a human still has to read the wording against the statute.${stillMissing}`;
  }
  const found = f.elements.filter((e) => e.found);
  ...
  return (
    lead +
    (found.length
      ? `On a keyword check, the clause covers ${found.length} of ${f.elements.length} elements. Not located anywhere in the policy: ${missing}.`
      : `A related clause exists, but none of the elements this obligation needs were located by keyword: ${missing}.`) +
    stillMissing
  );
}
